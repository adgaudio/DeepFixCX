...
from deepfixcx.models import (
    vip, MedianPool2d,
    get_effnetv2, get_resnet, get_densenet, get_efficientnetv1,
    get_DeepFixCXEnd2End, get_DeepFixCXEnd2End_v2, DeepFixCXMLP, UnetD,
    DeepFixCXImg2Img, MDMLP  # note: timm.create_model is also used.
)
from torchvision.transforms import GaussianBlur
from deepfixcx.dsets import (
...
MODELS = {
    ...
    ('waveletmlp', str, str, str, str, str, str, str): (
        lambda mlp_channels, in_ch, out_ch, wavelet_levels, patch_size, in_ch_mul, mlp_depth: get_DeepFixCXEnd2End(
    ...
    ('waveletmlpV2', str, str, str, str, str, str): (
        lambda in_ch, out_ch, wavelet, wavelet_levels, patch_size, patch_features: get_DeepFixCXEnd2End(
            int(in_ch), int(out_ch),
    ...
    ('waveletmlp_bn', str, str, str, str, str, str, str, str): (
        lambda in_ch, out_ch, wavelet, wavelet_levels, patch_size, patch_features, zero_mean, normalization: get_DeepFixCXEnd2End(
            int(in_ch), int(out_ch),
            in_ch_multiplier=1, wavelet=wavelet,
            wavelet_levels=int(wavelet_levels), wavelet_patch_size=int(patch_size),
    ...
    ('attn_test', str, str, str, str): (
        lambda attn, out_ch, wavelet_levels, patch_size: get_DeepFixCXEnd2End(
    ...
    ('deepfixcx_v1', str, str, str): (
        lambda out_ch, wavelet_levels, patch_size: get_DeepFixCXEnd2End(
            1, int(out_ch), in_ch_multiplier=1, wavelet='coif2',
            wavelet_levels=int(wavelet_levels), wavelet_patch_size=int(patch_size),
            patch_features='l1',
            mlp_depth=1, mlp_channels=300, mlp_fix_weights='none', mlp_activation=None,
    ...
    ('deepfixcx_v1', str, str, str, str): (
        lambda out_ch, wavelet_levels, patch_size, adaptive: get_DeepFixCXEnd2End(
            1, int(out_ch), in_ch_multiplier=1, wavelet='coif2',
            wavelet_levels=int(wavelet_levels), wavelet_patch_size=int(patch_size),
            patch_features='l1',
            mlp_depth=1, mlp_channels=300, mlp_fix_weights='none', mlp_activation=None,
    ...
    ('deepfixcx_v1', str, str, str, str, str): (
        lambda out_ch, wavelet_levels, patch_size, adaptive, wavelet: get_DeepFixCXEnd2End(
    ...
    ('deepfixcx_v1', str, str, str, str, str, str): (
        lambda out_ch, wavelet_levels, patch_size, adaptive, wavelet, normalization: get_DeepFixCXEnd2End(
            1, int(out_ch), in_ch_multiplier=1, wavelet=wavelet,
            wavelet_levels=int(wavelet_levels), wavelet_patch_size=int(patch_size),
            patch_features='l1',
            mlp_depth=1, mlp_channels=300, mlp_fix_weights='none', mlp_activation=None,
    ...
    ('deepfixcx_v2', str, str, str, str, str, str, str, str): (
    ...
    ('deepfixcx_cervical', str, str): (lambda J, P: get_DeepFixCXEnd2End(
    ...
    ('deepfixcx_resnet18', str, str, str, str, str): (lambda in_ch, out_ch, J, Ph, Pw: T.nn.Sequential(
        DeepFixCXImg2Img(in_channels=int(in_ch), J=int(J), P=(int(Ph), int(Pw)),
                       wavelet='db1', patch_features='l1',
                       restore_orig_size=False,
                       ),
        get_resnet('resnet18', 'untrained', int(in_ch), int(out_ch)),
    )),
    ('deepfixcx_densenet121', str, str, str, str, str): (lambda in_ch, out_ch, J, Ph, Pw: T.nn.Sequential(
        DeepFixCXImg2Img(in_channels=int(in_ch), J=int(J), P=(int(Ph), int(Pw)),
                       wavelet='db1', patch_features='l1',
    ...
    ('deepfixcx_volo_d1_224', str, str, str, str, str): (lambda in_ch, out_ch, J, Ph, Pw: T.nn.Sequential(
        DeepFixCXImg2Img(in_channels=int(in_ch), J=int(J), P=(int(Ph), int(Pw)),
                       wavelet='db1', patch_features='l1',
                       restore_orig_size=False,
                       ),
    ...
    ('deepfixcx_efficientnetv2_m', str, str, str, str, str): (lambda in_ch, out_ch, J, Ph, Pw: T.nn.Sequential(
        DeepFixCXImg2Img(in_channels=int(in_ch), J=int(J), P=(int(Ph), int(Pw)),
                       wavelet='db1', patch_features='l1',
    ...
    ('deepfixcx_efficientnet-b0', str, str, str, str, str): (lambda in_ch, out_ch, J, Ph, Pw: T.nn.Sequential(
        DeepFixCXImg2Img(in_channels=int(in_ch), J=int(J), P=(int(Ph), int(Pw)),
                       wavelet='db1', patch_features='l1',
                       restore_orig_size=False, min_size=(64,64)
                       ),
    ...
    ('deepfixcx_vip_s7', str, str, str, str, str): (
    ...
    ('deepfixcx_coatnet_1_224', str, str, str, str, str): (
        lambda in_ch, out_ch, J, Ph, Pw: T.nn.Sequential(
            DeepFixCXImg2Img(
    ...
    #  ('waveletres18v2', str, str, str): lambda pretrain, in_ch, out_ch: (
        #  DeepFixCXCompression(levels=8, wavelet='coif1', patch_size=1),
        #  R2(pretrain, int(in_ch), int(out_ch))),
    ('deepfixcx_mdmlp_320', str, str ,str, str, str):
        lambda in_ch, out_ch, J, Ph, Pw: T.nn.Sequential(
            DeepFixCXImg2Img(
                in_channels=int(in_ch), J=int(J), P=(int(Ph), int(Pw)),
    ...
    ('deepfixcx_mdmlp_patch14_lap7_dim64_depth8_224', str, str, str, str, str): (
        lambda in_ch, out_ch, J, Ph, Pw: T.nn.Sequential(
            DeepFixCXImg2Img(
                in_channels=int(in_ch), J=int(J), P=(int(Ph), int(Pw)),
                wavelet='db1', patch_features='l1', restore_orig_size=True),
...
DSETS = {
    ...
    ('food101', str, str): (
        lambda J, P: get_dset_food101_deepfixcxed(int(J), int(P))),
}
...
class RegularizedLoss(T.nn.Module):
    def __init__(self, model, lossfn, regularizer_spec:str):
        ...
        elif regularizer_spec.startswith('deepfixcxmlp:'):
            lbda = float(regularizer_spec.split(':')[1])
            self.regularizer = lambda *y: (
                float(lbda) * DeepFixCXMLP.get_VecAttn_regularizer(model))
        else:
...
class TrainOptions:
    ...
    loss_reg:str = 'none'  # Optionally add a regularizer to the loss.  loss + reg.  Accepted values:  "none", "deepfixcxmlp:X" where X is a positive float denoting the lambda in l1 regularizer
