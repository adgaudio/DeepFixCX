...
class MLP(T.nn.Module):
    ...
    def __init__(self, in_ch, out_ch, depth=8, mid_ch=None,
        ...
        lst.append(T.nn.Sequential(
            T.nn.Flatten(), T.nn.Linear(in_ch, mid_ch, bias=True),T.nn.LayerNorm(mid_ch),T.nn.CELU()))
        # add linear -> celu layers
        for _ in range(depth-1):
            lst.extend(T.nn.Sequential(T.nn.Linear(mid_ch, mid_ch, bias=True),T.nn.LayerNorm(mid_ch),T.nn.CELU()))
