...
source <(cat ./bin/experiments.sh|head -n-5 |tail -n+2)
...
mkdir -p $RAMDISK/results/anonymity_scores/cache/
C22 | parallel -j 1 "echo  run {} --device cpu --cache_dir $RAMDISK/results/anonymity_scores/cache/{experiment_id} --parallelization 40" | parallel -j 1
