...
def plot_accuracy_DNNs():
    ...
    chexpert_baselines = pd.DataFrame([
        ...
        ("DenseNet121 vs $DeepFix$", 'DNN', 0.868),  # 4.C8
        ("DenseNet121 vs $DeepFix$",
         '$DeepFix$ (J=1 P=115) -> DNN', 0.876),  # 2.C28
        # ("DenseNet121 vs $DeepFix$", '$DeepFix$ (J=1 P=160) -> DNN', 0.881),  # 2.C28

        ("EfficientNet-b0 vs $DeepFix$", 'DNN', 0.874),  # 4.C8
        ("EfficientNet-b0 vs $DeepFix$",
         '$DeepFix$ (J=1 P=160) -> DNN', 0.871),  # 2.C30
        # ("EfficientNet-b0 vs $DeepFix$", '$DeepFix$ (J=1 P=115) -> DNN', 0.877),  # 2.C30

        ("EfficientNetV2_m vs $DeepFix$", 'DNN', .869),  # 4.C8
        ("EfficientNetV2_m vs $DeepFix$",
         '$DeepFix$ (J=1 P=115) -> DNN', .869),  # 2.C33

        ('MDMLP_320 vs $DeepFix$', 'DNN', .831),
        ('MDMLP_320 vs $DeepFix$', '$DeepFix$ (J=1 P=79) -> DNN', .848),  # 2.C35
        # ('MDMLP_320 vs $DeepFix$', '$DeepFix$ (J=1 P=37) -> DNN', .854),  # 2.C35

        ('VOLO_d1_224 vs $DeepFix$', 'DNN', .805),
        ('VOLO_d1_224 vs $DeepFix$', '$DeepFix$ (J=1 P=115) -> DNN', .837),  # 2.C29
        # ('VOLO_d1_224 vs $DeepFix$', '$DeepFix$ (J=2 P=79) -> DNN', .843),  # 2.C29

        #
        ('CoAtNet_224 vs $DeepFix$', 'DNN', .828),
        ('CoAtNet_224 vs $DeepFix$', '$DeepFix$ (J=1 P=160) -> DNN', .831),  # 2.C36
        # ('CoAtNet_224 vs $DeepFix$', '$DeepFix$ (J=1 P=37) -> DNN', .843),  # 2.C36

        ('VIP_s7 vs $DeepFix$', 'DNN', .801),  # 4.C8
        ('VIP_s7 vs $DeepFix$', '$DeepFix$ (J=1 P=115) -> DNN', .801),  # 2.C33
        # not evaluated on all models.

        # ('ResNet18 vs $DeepFix$', 'DNN', 0.864),  # 4.C8
        # ('ResNet18 vs $DeepFix$', '$DeepFix$ (J=1 P=115) -> DNN', .845),  # 2.C33
    ...
    # barplot.set_title("$DeepFix$ Improves Prediction Performance of DNNs")
...
def plot_accuracy_blur_median(plot=True):
    df1 = pd.DataFrame([
        ('K=4 vs\nP=80', 'Blur', 'Blur, K={K}',  .856), (
            'K=4 vs\nP=80', 'Blur', r'closest $DeepFix$, J=1 P={P}', .883),
        ('K=8 vs\nP=40', 'Blur', 'Blur, K={K}',  .854), (
            'K=8 vs\nP=40', 'Blur', r'closest $DeepFix$, J=1 P={P}', .855),
        ('K=16 vs\nP=20', 'Blur', 'Blur, K={K}', .792), (
            'K=16 vs\nP=20', 'Blur', r'closest $DeepFix$, J=1 P={P}', .843),
        ('K=32 vs\nP=10', 'Blur', 'Blur, K={K}', .757), (
            'K=32 vs\nP=10', 'Blur', r'closest $DeepFix$, J=1 P={P}', .826),
        ('K=64 vs\nP=5', 'Blur', 'Blur, K={K}', .741),  (
            'K=64 vs\nP=5', 'Blur', r'closest $DeepFix$, J=1 P={P}', .785),
        # \hline
        ('K=3 vs\nP=80', 'Median', 'Median, K={K}',  .859), (
            'K=3 vs\nP=80', 'Median', r'closest $DeepFix$, J=1 P={P}',  .883),
        ('K=5 vs\nP=80', 'Median', 'Median, K={K}',  .854), (
            'K=5 vs\nP=80', 'Median', r'closest $DeepFix$, J=1 P={P}',  .883),
        ('K=9  vs\nP=40', 'Median', 'Median, K={K}', .858), (
            'K=9  vs\nP=40', 'Median', r'closest $DeepFix$, J=1 P={P}', .855),
        ('K=15 vs\nP=40', 'Median', 'Median, K={K}', .850), (
            'K=15 vs\nP=40', 'Median', r'closest $DeepFix$, J=1 P={P}', .855),
        ('K=25 vs\nP=18', 'Median', 'Median, K={K}', .831), (
            'K=25 vs\nP=18', 'Median', r'closest $DeepFix$, J=1 P={P}', .840),
        ('K=50 vs\nP=6', 'Median', 'Median, K={K}', .800),  (
            'K=50 vs\nP=6', 'Median', r'closest $DeepFix$, J=1 P={P}', .791),
        ('K=75 vs\nP=4', 'Median', 'Median, K={K}', .761),  (
            'K=75 vs\nP=4', 'Median', r'closest $DeepFix$, J=1 P={P}', .766),
    ], columns=[
...
def plot_ssim_blur_median():
    experiments = [
        ...
        {'Experiment': 'K=4 vs\n J=1 P=80', 'Type': 'Blur',
            'Method': 'closest $DeepFix$, J=1 P={P}'},
...
def plot_compression_blur_median():
    ...
    for orig_img, _ in loader:
        ...
        instructions = [
            ...
            ('Median', (lambda img, K: median(img, K, min_size=(1, 1))),
             [3, 5, 9, 15, 25, 50, 75]),
            ('$DeepFix$', (lambda img, P: DeepFixImg2Img(1, J=1, P=P)(orig_img)),
    ...
    df2 = pd.concat({
        'Method': df1['Method'].str.extract('(Blur|Median|\$DeepFix\$)')[0],
    ...
    df2.loc[df2['Method'] == '$DeepFix$', 'K'] = None
    df2.loc[df2['Method'] != '$DeepFix$', 'P'] = None
    ...
    df3b = pd.concat({
        'Method': df3a['Method'].str.extract('(Blur|Median|\$DeepFix\$)')[0],
        'K': df3a['Experiment'].str.extract('K=(\d+)')[0],
        'P': df3a['Experiment'].str.extract('P=(\d+)')[0],
        'SSIM': df3a['SSIM'],
    }, axis=1)
    df3b.loc[df3b['Method'] == '$DeepFix$', 'K'] = None
    df3b.loc[df3b['Method'] != '$DeepFix$', 'P'] = None
    df3b = df3b.melt(['Method', 'SSIM'], ['K',  'P'], 'Param').dropna()
    #
    df4a = pd.read_csv('results/plots/ssim_Median_vs_DeepFix.csv')
    df4b = pd.concat({
        'Method': df4a['Method'].str.extract('(Median|Median|\$DeepFix\$)')[0],
        'K': df4a['Experiment'].str.extract('K=(\d+)')[0],
        'P': df4a['Experiment'].str.extract('P=(\d+)')[0],
        'SSIM': df4a['SSIM'],
    }, axis=1)
    df4b.loc[df4b['Method'] == '$DeepFix$', 'K'] = None
    df4b.loc[df4b['Method'] != '$DeepFix$', 'P'] = None
    ...
    savefp =
    './results/plots/imr_ssim_acc_blur_median_deepfix.png'
...
def _plot_nimble(csv_files, savefp,
    ...
    df = pd.DataFrame(results, columns=[
        ...
        'with $DeepFix$', deepfix_name_in_legend])  # 2nd and 4th column are SPE
    # print(df.head(1))
...
def plot_nimble_blur_median():
    blur_csv_files = [
        ['K=4', 'results/2.C31.K=4/perf.csv',
         '$DeepFix$ (J=1 P=80)', 'results/2.C31b.J=1.P=80/perf.csv'],
        ['K=8', 'results/2.C31.K=8/perf.csv',
         '$DeepFix$ (J=1 P=40)', 'results/2.C31b.J=1.P=40/perf.csv'],
        ['K=16', 'results/2.C31.K=16/perf.csv',
         '$DeepFix$ (J=1 P=20)', 'results/2.C31b.J=1.P=20/perf.csv'],
        ['K=32', 'results/2.C31.K=32/perf.csv',
         '$DeepFix$ (J=1 P=10)', 'results/2.C31b.J=1.P=10/perf.csv'],
        ['K=64', 'results/2.C31.K=64/perf.csv',
         '$DeepFix$ (J=1 P=5)', 'results/2.C31b.J=1.P=5/perf.csv'],
    ]
    median_csv_files = [
        ['K=3', 'results/2.C32.K=3/perf.csv', '$DeepFix$ (J=1 P=80)',
         'results/2.C31b.J=1.P=80/perf.csv'],
        ['K=5', 'results/2.C32.K=5/perf.csv', '$DeepFix$ (J=1 P=80)',
         'results/2.C31b.J=1.P=80/perf.csv'],
        ['K=9', 'results/2.C32.K=9/perf.csv', '$DeepFix$ (J=1 P=40)',
         'results/2.C31b.J=1.P=40/perf.csv'],
        ['K=15', 'results/2.C32.K=15/perf.csv', '$DeepFix$ (J=1 P=40)',
         'results/2.C31b.J=1.P=40/perf.csv'],
        ['K=25', 'results/2.C32.K=25/perf.csv', '$DeepFix$ (J=1 P=18)',
         'results/2.C31b.J=1.P=18/perf.csv'],
        ['K=50', 'results/2.C32.K=50/perf.csv', '$DeepFix$ (J=1 P=6)',
         'results/2.C31b.J=1.P=6/perf.csv'],
        ['K=75', 'results/2.C32.K=75/perf.csv',
            '$DeepFix$ (J=1 P=4)', 'results/2.C31b.J=1.P=4/perf.csv'],
    ]
    df1 = _plot_nimble(
        median_csv_files, 'results/plots/nimble_median_vs_deepfix.png',
        'Median, K={K}', 'closest $DeepFix$, J=1, P={P}',
    ...
    df2 = _plot_nimble(
        ...
        'Blur, K={K}', 'closest $DeepFix$, J=1, P={P}',
        hline=(107.644, 'DNN (EfficientNet-b0, no privatized compression)'))  # number from 4.C8 efficientnet SPE.min()
    return (df1, df2)
